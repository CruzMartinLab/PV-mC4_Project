function [yi, yii,event_minima, event_times, event_amp, event_rise1090, event_decaytau, event_charge, all_fits] = find_IPSCs_PVIN(trace)
...
%this compares the slope of the 20 (OG) points behind the peak (should be steep
%rise) and the 40 (OG) points after the peak (should be more gradual slope).
%This helps get rid of noise (bigger dips where both rise and decay are
%roughly equal
for ii = 2:length(to_plot)
   if isnan(to_plot(ii,1))
       continue
       ...
       if to_plot(ii,3) < 15 
           ...
           behind_point_idx = working_peak_idx - 40;
           behind_point = y(behind_point_idx);

           ahead_point_idx = working_peak_idx + 100;
           ahead_point = y(ahead_point_idx);

           behind_slope = -((behind_point - working_peak)/(behind_point_idx - working_peak_idx));
           ahead_slope = ((ahead_point - working_peak)/(ahead_point_idx - working_peak_idx));

           %behind_slope < ahead_slope*2 %can use this to check

           if behind_slope < ahead_slope*2
                to_plot(ii,1) = NaN;
                to_plot(ii,2) = NaN;
                to_plot(ii,3) = NaN;
           end
