...
%                load(fullfile(logs(i).folder,'mouse_sex.mat'));
           ...
           interactions = zeroMaze(NUM(:,[center,topleftin,topleftout,toprightin,toprightout,bottomleftin,bottomleftout,bottomrightin,bottomrightout]),NUM(:,[snout, head, centroid]), VideoReader(fullfile(logs(i).folder,'behavCam1.avi')), VideoWriter(fullfile(logs(i).folder,'behavCam1_ROI_11_12.avi')),timestamp.behavecam(:,3), thresh);
...
function interactions = zeroMaze(coord,snout, oldv, newv, time, thresh)
    ...
    %Set bounds where the closed arms open, NEED TO MAKE IT HIT TOP OF IN
    top_bound_left = floor((coord(1,4)+coord(1,6))/2); 
    top_bound_right = floor((coord(1,8)+coord(1,10))/2); 
    bottom_bound_left=floor((coord(1,12)+coord(1,14))/2); 
    bottom_bound_right=floor((coord(1,16)+coord(1,18))/2); 
...
%      time_dif = ((time') - time2(1:m))/1000; 
    ...
    for i = 2:(m+1)
        ...
        if (interactions{i,10} >=0.0005) %threshold to see if mouse is moving between frames
            ...
            if(centroid(2) < top_bound_left) && (centroid(1)<center(1))
               interactions{i,2} = true; %Closed
               interactions{i,4} = true; %Top
            elseif (centroid(2) < top_bound_right) && (centroid(1)>center(1))
               interactions{i,2} = true; %Closed
               interactions{i,4} = true; %Top
            elseif (centroid(2) > bottom_bound_left) && (centroid(1)<center(1))
                interactions{i,2} = true; %Closed
                interactions{i,5} = true; %Bottom
            elseif (centroid(2) > bottom_bound_right) && (centroid(1)>center(1))
                interactions{i,2} = true; %Closed
                interactions{i,5} = true; %Bottom
            else
            ...
            if headpoke(2)>top_bound_left && headpoke(1)<center(1)
                if headpoke(2)<bottom_bound_left
                interactions{i,12}=true;
                end
            end
            
             if headpoke(2)>top_bound_right && headpoke(1)>center(1)
                if headpoke(2)<bottom_bound_right
                interactions{i,12}=true;
                end
             end
            
            if snoutpoke(2)>top_bound_left && snoutpoke(1)<center(1)
                if snoutpoke(2)<bottom_bound_left
                interactions{i,12}=true;
                end
            end
            
             if snoutpoke(2)>top_bound_right && snoutpoke(1)>center(1)
                if snoutpoke(2)<bottom_bound_right
                interactions{i,12}=true;
                end
            end
...
%                         end
            ...
            %Mark bounds
            frame((top_bound_left-1):(top_bound_left+1),floor(coord(1,5)):floor(coord(1,3)),1) = 175;
            frame((top_bound_left-1):(top_bound_left+1),floor(coord(1,5)):floor(coord(1,3)),2) = 54;
            frame((top_bound_left-1):(top_bound_left+1),floor(coord(1,5)):floor(coord(1,3)),3) = 60;

          
            frame((top_bound_right-1):(top_bound_right+1),floor(coord(1,7)):floor(coord(1,9)),1) = 175;
            frame((top_bound_right-1):(top_bound_right+1),floor(coord(1,7)):floor(coord(1,9)),2) = 54;
            frame((top_bound_right-1):(top_bound_right+1),floor(coord(1,7)):floor(coord(1,9)),3) = 60;
            
            frame((bottom_bound_left-1):(bottom_bound_left+1),floor(coord(1,13)):floor(coord(1,11)),1) = 175;
            frame((bottom_bound_left-1):(bottom_bound_left+1),floor(coord(1,13)):floor(coord(1,11)),2) = 54;
            frame((bottom_bound_left-1):(bottom_bound_left+1),floor(coord(1,13)):floor(coord(1,11)),3) = 60;

            frame((bottom_bound_right-1):(bottom_bound_right+1),floor(coord(1,15)):floor(coord(1,17)),1) = 175;
            frame((bottom_bound_right-1):(bottom_bound_right+1),floor(coord(1,15)):floor(coord(1,17)),2) = 54;
            frame((bottom_bound_right-1):(bottom_bound_right+1),floor(coord(1,15)):floor(coord(1,17)),3) = 60;
...
blin=[fixed(11) fixed(12)];
blout=[fixed(13) fixed(14)];
